...
export default function AuthForm() {
  ...
  useEffect(() => {
    ...
    if (errorMessage) {
      ...
      // If the error is about database issues, show the manual signup form
      if (errorMessage.includes('Database error')) {
        setShowManualSetup(true);
        setIsSignIn(false); // Switch to signup mode
      }
        ...
        <div className="p-3 bg-red-50 text-red-700 rounded-md text-sm">
          ...
          {error.includes('Database error') && (
            <p className="mt-2 text-sm text-gray-700">
              This could be a temporary issue. Please try again or use
      ...
      <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
        ...
        {error && error.includes('Database error') && (
          <div className="mt-4 text-sm text-indigo-600">
            <p>
              Create an account with email/password instead to avoid this issue.
            </p>
          </div>
        )}
